...
const AssetPanel: React.FC<AssetPanelProps> = ({ 
  ...
  /**
   * Reparents an object to a new group (or root) while maintaining its world transform.
   * This ensures the object visually stays in place (doesn't jump to group center).
   */
  const handleReparent = (objId: string, newGroupId: string | undefined) => {
    ...
    // Helper: Construct Matrix4 from array transforms
    ...
    // 1. Capture absolute world transform of the object
    let objectWorldMatrix = createMatrix(obj.position, obj.rotation, obj.scale);

    // If it was inside a group, apply the old parent's transform to get true world coords
    if (obj.groupId) {
      ...
      if (oldParent) {
        ...
        objectWorldMatrix.premultiply(parentMatrix); // World = Parent * Local
      }
    }

    // 2. Calculate new local transform relative to the new parent
    const newLocalMatrix = new THREE.Matrix4(); // Identity
    ...
    if (newGroupId) {
      ...
      if (newParent) {
        ...
        // Local = InvParent * World
        newLocalMatrix.multiplyMatrices(inverseNewParentMatrix, objectWorldMatrix);
      } else {
        // Fallback: if group not found, keep world matrix
        newLocalMatrix.copy(objectWorldMatrix);
      }
    } else {
      // Moving to root: Local is World
      newLocalMatrix.copy(objectWorldMatrix);
    }

    // 3. Decompose to get the new local transform values
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
               <button onClick={onAddTerrain} className="flex flex-col items-center justify-center gap-2 aspect-square bg-[#0a0a0a] border border-[#222] rounded-md hover:border-green-500 hover:bg-green-600/5 transition-all group">
                  ...
                  <span className="text-[9px] font-black uppercase text-gray-600 group-hover:text-white text-center px-1">Terrain</span>
               </button>
            </div>
                  ...
                  {selectedObject.type === 'terrain' && selectedObject.terrainData && (
                     <div className="space-y-4">
                        <h3 className="text-[10px] font-bold text-green-400 uppercase tracking-widest flex items-center gap-2">
                           <Mountain size={12} /> Terrain Generator
                        </h3>
                        <div className="bg-[#0a0a0a] border border-[#222] rounded-lg p-3 space-y-3">
                           <div>
                              <div className="flex justify-between text-[8px] text-gray-500 uppercase font-black mb-1">
                                 <span>Elevation</span> <span className="text-blue-400">{selectedObject.terrainData.elevation}u</span>
                              </div>
                              <input type="range" min="0" max="10" step="0.1" value={selectedObject.terrainData.elevation} onChange={(e) => onUpdate(selectedObject.id, { terrainData: { ...selectedObject.terrainData!, elevation: parseFloat(e.target.value) } })} className="w-full h-1 bg-[#222] rounded-lg appearance-none accent-blue-600" />
                           </div>
                           
                           <div>
                              <div className="flex justify-between text-[8px] text-gray-500 uppercase font-black mb-1">
                                 <span>Noise Scale</span> <span className="text-blue-400">{selectedObject.terrainData.roughness}</span>
                              </div>
                              <input type="range" min="0.1" max="5" step="0.1" value={selectedObject.terrainData.roughness} onChange={(e) => onUpdate(selectedObject.id, { terrainData: { ...selectedObject.terrainData!, roughness: parseFloat(e.target.value) } })} className="w-full h-1 bg-[#222] rounded-lg appearance-none accent-blue-600" />
                           </div>

                           <div>
                              <div className="flex justify-between text-[8px] text-gray-500 uppercase font-black mb-1">
                                 <span>Water Level</span> <span className="text-blue-400">{selectedObject.terrainData.waterLevel}</span>
                              </div>
                              <input type="range" min="-1" max="1" step="0.05" value={selectedObject.terrainData.waterLevel} onChange={(e) => onUpdate(selectedObject.id, { terrainData: { ...selectedObject.terrainData!, waterLevel: parseFloat(e.target.value) } })} className="w-full h-1 bg-[#222] rounded-lg appearance-none accent-blue-600" />
                           </div>
                           ...
                           <div>
                              ...
                              <input type="range" min="16" max="128" step="16" value={selectedObject.terrainData.segments} onChange={(e) => onUpdate(selectedObject.id, { terrainData: { ...selectedObject.terrainData!, segments: parseInt(e.target.value) } })} className="w-full h-1 bg-[#222] rounded-lg appearance-none accent-blue-600" />
                           </div>
                           ...
                           <button onClick={() => onUpdate(selectedObject.id, { terrainData: { ...selectedObject.terrainData!, seed: Math.random() } })} className="w-full py-2 bg-[#1a1a1a] hover:bg-[#252525] border border-[#333] rounded text-[9px] text-gray-300 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
                              <Sparkles size={10} /> Randomize Seed
                           </button>
                        </div>
                     </div>
                  )}
